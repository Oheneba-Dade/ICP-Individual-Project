...
public class TestRoute {
	...
	/**
	 * The method takes in a source and destination and performs a breadth-first search to return an ArrayList of Strings that represent the path from the
		...
		while (frontier.size() > 0) {
			...
			if (successorStates != null) {
				for (Route successorState : successorStates) {
					Node child = new Node(node, successorState.getDestinationAirportCode(), successorState.getStops(), successorState.getAirlineCode());
					String destinationCityAndCountry = Airport.airportCodeToCityAndCountryMap.get(child.getAirportCode());
	...
	public static void main(String[] args) {
		try {
			...
			while (routeLine != null) {
				...
				Route route = new Route(routeInfo[0], routeInfo[1], routeInfo[4], Integer.parseInt(routeInfo[7]));
			...
			BufferedReader inputFileReader = new BufferedReader(new FileReader(String.valueOf(inputFileName)));
			StringBuilder outputFileName = new StringBuilder(inputFileName.delete(inputFileName.length() - 4, inputFileName.length()));
